...
export async function GET() {
  ...
  const pdfSvcUrl  = process.env.PDF_SERVICE_URL

  // Report token & PDF service status without exposing values
  const tokenInfo  = !token
    ? 'NOT SET (empty/undefined)'
    : `SET — length ${token.length}, starts with "${token.slice(0, 10)}…"`

  const pdfSvcInfo = !pdfSvcUrl
    ? 'NOT SET — PDF generated locally on Vercel'
    : `SET — "${pdfSvcUrl}" (PDF generated by external service — needs HUGGINGFACE_TOKEN there too)`

  if (!token) {
    return NextResponse.json({ ok: false, tokenInfo, pdfSvcInfo }, { status: 500 })
  }
  ...
  try {
    ...
    return NextResponse.json({
      ok: false, tokenInfo, pdfSvcInfo, status, contentType, elapsed: `${elapsed}ms`,
    ...
    return NextResponse.json({
      ok: false, tokenInfo, pdfSvcInfo, status, elapsed: `${elapsed}ms`,
      exception: message,
    }, { status: 502 })
  }
}
